...
export default function EssentialTools() {
  ...
  const tools = [
    ...
    { name: "CSS", description: "User Interface", image: "/images/CSS3.png" },
    ...
    { name: "NestJS", description: "Node Framework", image: "/images/NestJs.svg" },
  ...
  return (
    <section id="tools" className="container pt-10 md:pt-20 w-full px-6">
      <h2 className={`text-center text-2xl sm:text-3xl font-bold mb-4 leading-tight text-transparent bg-clip-text bg-gradient-to-r ${theme === "light"
      ...
      <p className={`mb-8 max-w-2xl mx-auto text-center text-sm sm:text-base leading-tight text-transparent bg-clip-text bg-gradient-to-r ${theme === "light"
        ? "from-[#202016] to-gray-900"
      ...
      <div className="grid grid-cols-1 min-[300px]:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {tools.map((tool) => (
          <div key={tool.name}
            ...
            <p className={`text-xs sm:text-sm mt-1 ${theme === "light" ? "text-gray-800" : "text-gray-300"}`}>
              {tool.description}
            </p>
          </div>
        ))}
